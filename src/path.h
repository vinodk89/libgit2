...
/**
 * Validate an on-disk path.  This ensures that the given path is valid
 * for the operating system/platform.  This will ensure that the given
 * absolute path is smaller than MAX_PATH on Windows.
 *
 * This should be used for paths that are *not* in the working directory
 * (and thus do not honor `core.longpaths`).
 */
GIT_INLINE(int) git_path_validate_ondisk_with_suffix(
	const char *path,
	size_t path_len,
	size_t suffix_len)
{
	...
	if (GIT_ADD_SIZET_OVERFLOW(&total_len, path_len, suffix_len) ||
	    total_len > MAX_PATH) {
...
GIT_INLINE(int) git_path_validate_ondisk(const char *path, size_t path_len)
{
	return git_path_validate_ondisk_with_suffix(path, path_len, 0);
}
